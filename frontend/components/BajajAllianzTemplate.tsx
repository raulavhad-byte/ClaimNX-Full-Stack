  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      <div className="flex border-l border-t border-black bg-white shrink-0">
        {chars.map((char, i) => (
          <div key={i} className="w-[10.5px] h-[12px] shrink-0 border-r border-b border-black flex items-center justify-center text-[8px] font-black text-[#00338d]">
            {char === ' ' ? '' : char}
          </div>
        ))}
      </div>
...
const DateGrid: React.FC<{ value: string; label?: string }> = ({ value, label }) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex">
        {sequence.map((char, i) => (
          <div key={i} className={`w-[10.5px] h-[12px] shrink-0 border border-black flex items-center justify-center text-[8px] font-black text-[#00338d] bg-white ${i === 1 || i === 3 ? 'mr-1.5' : '-ml-[1px]'}`}>
            {char}
          </div>
        ))}
      </div>
...
const UnderlineField: React.FC<{ label: string; value: string; className?: string }> = ({ label, value, className = "" }) => (
  <div className={`flex items-end border-b border-black/20 pb-0.5 ${className}`}>
    ...
    <span className="text-[9px] font-black text-[#00338d] uppercase flex-1 truncate">{value}</span>
  </div>
);
