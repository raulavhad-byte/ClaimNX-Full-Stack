  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      <div className="flex border-l border-t border-black bg-white">
        {chars.map((char, i) => (
          <div key={i} className="w-[11px] h-[12px] shrink-0 border-r border-b border-black flex items-center justify-center text-[8.5px] font-black text-[#00338d]">
            {char === ' ' ? '' : char}
          </div>
        ))}
      </div>
...
const DateGrid: React.FC<{ value: string; label?: string }> = ({ value, label }) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex">
        {sequence.map((char, i) => (
          <div key={i} className={`w-[11px] h-[12px] shrink-0 border border-black flex items-center justify-center text-[8.5px] font-black text-[#00338d] bg-white ${i === 1 || i === 3 ? 'mr-1' : '-ml-[1px]'}`}>
            {char}
          </div>
        ))}
      </div>
