  ...
  return (
    <div className={`flex flex-col ${className}`}>
      ...
      <div className="flex border-l border-t border-[#6B0F1A]">
        {chars.map((char, i) => (
<div key={i} className="w-[11.5px] h-[13px] shrink-0 border-r border-b border-[#6B0F1A] flex items-center justify-center text-[9px] font-black text-[#00338d] bg-white">
            {char === ' ' ? '' : char}
          </div>
        ))}
      </div>
...
const DateGrid: React.FC<{ value: string; label?: string }> = ({ value, label }) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex">
        {sequence.map((char, i) => (
<div key={i} className={`w-[11.5px] h-[13px] shrink-0 border border-[#6B0F1A] flex items-center justify-center text-[9px] font-black text-[#00338d] bg-white ${i === 1 || i === 3 ? 'mr-1.5' : '-ml-[1px]'}`}>
            {char}
          </div>
        ))}
      </div>
