-- ClaimNX Phase 11 Reporting & BI command-persistence validation (READ ONLY).
-- Run only after:
--   1. 20260807235000_create_reporting_base_schema.sql
--   2. 20260808000000_create_reporting_command_persistence_functions.sql
...
SELECT
    ...
    to_regprocedure('public.set_report_definition_status(uuid,uuid,integer,uuid,uuid)') IS NOT NULL
        AS definition_status_command_exists,
    to_regprocedure('public.set_report_schedule_status(uuid,uuid,integer,uuid,uuid)') IS NOT NULL
        AS schedule_status_command_exists,
    to_regprocedure('public.set_report_execution_status(uuid,uuid,integer,uuid,uuid)') IS NOT NULL
        AS execution_status_command_exists,
    to_regprocedure('public.soft_delete_report_schedule(uuid,uuid,integer,uuid)') IS NOT NULL
        AS schedule_soft_delete_command_exists;
