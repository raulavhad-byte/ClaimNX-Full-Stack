...
export class YahooProvider implements IMailProvider {
  ...
  async sendMessage(account: any, input: OutboundMailInput): Promise<OutboundMailResult> { try { const transport = await this.transport(account.id, account.email_address); const info = await transport.sendMail({ from: account.email_address, to: input.to.map((item) => item.address).join(', '), cc: input.cc?.map((item) => item.address).join(', '), bcc: input.bcc?.map((item) => item.address).join(', '), subject: input.subject, text: input.plainTextBody, html: input.htmlBody, attachments: (input.attachments || []).map((file) => ({ filename: file.filename, content: Buffer.from(file.contentBase64, 'base64'), contentType: file.contentType })) }); return { success: true, providerMessageId: info.messageId, internetMessageId: info.messageId }; } catch (error: any) { return { success: false, error: error?.message || 'Unable to send through Yahoo.' }; } }
